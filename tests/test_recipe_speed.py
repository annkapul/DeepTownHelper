...
def test_recipe_speed():
    ...
    assert str(c.RecipeSpeed(c.Recipe("steel_plate", count=10)).all) == "[" \
                                                                        "Steel Bar x -2.500 RPM, Steel Plate x 0.500 RPM]"
    assert str(c.RecipeSpeed(c.Recipe("steel_plate", count=100)).all) == "[" \
                                                                         "Steel Bar x -2.500 RPM, Steel Plate x 0.500 RPM]"
    # TODO Incorrwct values for 720
    assert str(c.RecipeSpeed(c.Recipe("steel_plate", count=720)).all) == "[" \
                                                                         "Steel Bar x -2.500 RPM, Steel Plate x 0.500 RPM]"

def test_sum_speed():
    prod1 = c.RecipeSpeed(c.Recipe("steel_plate", count=10)).all
    prod2 = c.RecipeSpeed(c.Recipe("steel_plate", count=1200)).all
    assert str(c.sum_items_by_rpm(prod1 + prod2)) == "[Steel Bar x -5.000 " \
                                                     "RPM, Steel Plate x " \
                                                     "1.000 RPM]"
...
def test_boosters():
        ...
        assert str(prod2.all) == "[Steel Bar x -10.000 RPM, Steel Plate x " \
                                 "2.000 RPM]"
